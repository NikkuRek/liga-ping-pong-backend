...
export class PlayerValidator {
  ...
  validatePlayer = [
    ...
    check("playerData.first_name", "El primer nombre debe contener solo letras y letras con acentos").isString().matches(/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/),

    check("playerData.last_name", "El apellido es obligatorio").not().isEmpty(),
    check("playerData.last_name", "El apellido debe contener solo letras y letras con acentos").isString().matches(/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/),
    ...
    check("playerData.id_career", "El id de la carrera debe ser un número").isNumeric(), // O isInt()

    check("playerData.id_tier", "El id del nivel es obligatorio").not().isEmpty(),
    check("playerData.id_tier", "El id del nivel debe ser un número").isNumeric(), // O isInt()
    ...
    check("playerData.available_days.*", "Cada día en la disponibilidad debe ser un número entero válido").optional().isInt({ gt: 0, lt: 6 }), // Asumiendo IDs de día del 1 al 5
 ];
  ...
  validateIfCIExist = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.log('--- validateIfCIExist Debug ---');
  ...
  validateIfPhoneIsUse = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        console.error("Error en validateIfPhoneIsUse:", error);
        return res.status(500).json({
